...
export function detectFramework(rootDir: string): string | undefined {
  ...
  const deps = new Set([...Object.keys((pkg["dependencies"] as Record<string,string> | undefined) ?? {}), ...Object.keys((pkg["devDependencies"] as Record<string,string> | undefined) ?? {})]);
  for (const [name, hints] of Object.entries(FRAMEWORKS)) {
    if (hints.some(h => deps.has(h) || existsSync(join(rootDir, `${h}.js`)) || existsSync(join(rootDir, `${h}.ts`)))) return name;
  }
  return undefined;
}

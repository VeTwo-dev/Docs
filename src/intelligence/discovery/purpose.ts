...
export function discoverPurpose(
  rootDir: string,
  fallbackDescription?: string,
): { summary: string; capabilities: string[]; audiences: string[] } {
  let readme = "";
  ...
  const capabilities = [...readme.matchAll(/^#{2,3}\s+(.+)$/gm)]
    .map((m) => m[1]!.trim())
    .slice(0, 6);
  ...
  return {
    summary: firstParagraph || fallbackDescription || "Documentation for this project.",
    capabilities,
    audiences,
  };
}

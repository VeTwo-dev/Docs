...
function detectProjectType(pkg: Record<string, unknown>, hasBin: boolean, isMonorepo: boolean, framework?: string): ProjectTypeExt {
...
export function discoverIdentity(rootDir: string, opts: { framework?: string; hasBin?: boolean; isMonorepo?: boolean } = {}): ProjectIdentity {
  ...
  return {
    ...
    repository: typeof pkg["repository"] === "string" ? pkg["repository"] : (pkg["repository"] as { url?: string } | undefined)?.url,
    homepage: pkg["homepage"] as string | undefined,
    bugsUrl: typeof pkg["bugs"] === "string" ? pkg["bugs"] : (pkg["bugs"] as { url?: string } | undefined)?.url,
    packageManager: pm,
    language: existsSync(join(rootDir, "tsconfig.json")) ? "typescript" : "javascript",
    framework,
    runtime: (pkg["engines"] as unknown as Record<string, string> | undefined)?.["node"] ? `Node.js ${(pkg["engines"] as unknown as Record<string,string>)["node"]}` : undefined,
